...
namespace ORFlow.Application.Tests.SurgeryRequests.Archive;

public class ArchiveSurgeryRequestHandlerTests
{
    [Fact]
    public async Task HandleAsync_WhenCompletedSurgeryRequestExists_ArchivesAndSaves()
    {
        SurgeryRequest surgeryRequest = CreateCompletedSurgeryRequest();
        ...
        ArchiveSurgeryRequestHandler handler =
            new ArchiveSurgeryRequestHandler(repository);

        SurgeryRequest? result =
            await handler.HandleAsync(surgeryRequest.SurgeryRequestId);

        Assert.NotNull(result);
        Assert.Equal(RequestStatus.Archived, result.RequestStatus);
    ...
    private static SurgeryRequest CreateCompletedSurgeryRequest()
    {
